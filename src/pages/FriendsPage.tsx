...
const FriendsPage = () => {
  ...
  const getUserEmail = async (userId: string): Promise<string> => {
    try {
      ...
      return data.display_name || data.email || `User ${userId.slice(0, 8)}...`;
    } catch (error) {
      return `User ${userId.slice(0, 8)}...`;
    }
  };
  ...
  const fetchFriends = async () => {
    ...
    const friendsWithEmails = await Promise.all(
      (acceptedFriends || []).map(async (friendship) => {
        ...
        const friendEmail = await getUserEmail(friendId);
        return { ...friendship, friendEmail };
      })
    );

    const sentWithEmails = await Promise.all(
      (sentRequests || []).map(async (request) => {
        const friendEmail = await getUserEmail(request.addressee);
        return { ...request, friendEmail };
      })
    );

    const receivedWithEmails = await Promise.all(
      (receivedRequests || []).map(async (request) => {
        const friendEmail = await getUserEmail(request.requester);
        return { ...request, friendEmail };
      })
    );

    setFriends(friendsWithEmails);
    setPendingRequests(sentWithEmails);
    setFriendRequests(receivedWithEmails);
  };
  ...
  const sendFriendRequest = async (e: React.FormEvent) => {
    ...
    try {
      // Look up user by email in user_profiles table
      ...
      if (lookupError || !targetUser) {
        setMessage({ type: 'error', text: 'User not found. Please check the email address.' });
        setLoading(false);
        return;
      }
      ...
      if (insertError) {
        ...
        setMessage({ 
          type: 'success', 
          text: `Friend request sent to ${targetUser.display_name || friendEmail}!` 
        });
        setFriendEmail('');
          ...
          <Stack spacing={2}>
            {friendRequests.map((request) => (
              <Box key={request.id} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Typography>{request.friendEmail}</Typography>
          ...
          <Stack spacing={2}>
            {friends.map((friend) => (
              <Box key={friend.id} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Typography>{friend.friendEmail}</Typography>
                <Button
                  size="small"
                  variant="outlined"
                  color="error"
